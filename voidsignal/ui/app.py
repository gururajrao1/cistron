"""
VoidSignal Virtual Cellular Laboratory — polished Streamlit dashboard.

Launch::

    streamlit run voidsignal/ui/app.py
"""
...
def _require_streamlit() -> Any:
    try:
        ...
        raise SystemExit(
            ...
            "Install with:  pip install 'voidsignal[ui]'"
        ) from exc
    return st
...
def _fingerprint(cfg: VoidSignalPipelineConfig) -> str:
    return cfg.model_dump_json()


def run_pipeline(cfg: VoidSignalPipelineConfig) -> Tuple[VoidSignalPipelineResult, CausalActivityGraph]:
    graph = load_activity_graph(cfg.preset)
    result = VoidSignalPipeline(cfg, graph=graph).run()
    return result, graph
...
def _top_regulator(result: VoidSignalPipelineResult) -> str:
...
def render_header(st: Any, result: Optional[VoidSignalPipelineResult]) -> None:
        ...
        <div class="vcl-header">
          <div class="vcl-brand-wrap">
            ...
            <div class="vcl-brand-text">
              <div class="vcl-brand-name">VoidSignal</div>
...
def render_control_dock(st: Any) -> Tuple[VoidSignalPipelineConfig, bool]:
...
def render_metric_strip(
    st: Any,
    result: VoidSignalPipelineResult,
    node_y: Dict[str, float],
    edge_f: Dict[str, float],
) -> None:
...
def render_intelligence_drawer(st: Any, result: VoidSignalPipelineResult) -> None:
...
def render_empty_state(st: Any, cfg: VoidSignalPipelineConfig) -> None:
...
def main() -> None:
    ...
    st.set_page_config(
        page_title="VoidSignal Laboratory",
        page_icon="🧪",
        layout="wide",
    ...
    result: Optional[VoidSignalPipelineResult] = st.session_state.pipeline_result
