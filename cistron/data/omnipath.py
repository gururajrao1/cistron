"""
OmniPath + SIGNOR 3.0 activity-flow ingestion for CISTRON.
...
exports ODE-ready :class:`~cistron.topology.SignalingNetwork` objects.
...
from cistron.components import KineticParameters, Protein, StructuralMetadata
...
def parse_activity_flow_rows(
    ...
    *,
    ...
    return CausalActivityGraph(
        ...
        edges=edges,
        ambiguous=ambiguous,
        provenance={"parser": "cistron.data.omnipath", "n_input_rows": len(rows)},
    )
    ...
    for src, tgt, sign, mod, mech in raw_edges:
        edges.append(
            ActivityFlowEdge(
                ...
                mechanism=mech,
                sources=["CISTRON-hypoxia-preset"],
                datasets=["synthetic"],
            )
        )
        for sym in (src, tgt):
