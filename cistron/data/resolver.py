    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=client_timeout) as session:
            async with session.get(
                url,
                headers={"User-Agent": "Cistron-Resolver/0.22", "Accept": "*/*"},
            ) as resp:
                if resp.status >= 400:
                    return None
...
def _preset_fallback_graph(profile_id: str, query: str) -> CausalActivityGraph:
    ...
    from cistron.data.presets import local_graph_cache
...
async def _fetch_omnipath_async(
    ...
    *,
    ...
    from cistron.data.omnipath import _parse_tsv, parse_activity_flow_rows
    ...
    from cistron.data.omnipath import build_omnipath_query
...
async def resolve_multisource_network_async(
    ...
    *,
    ...
    graph cache in ``cistron.data.presets`` is used.
    """
    from cistron.data import multisource as ms
    ...
    if not live_sources:
        ...
        provenance = {
            ...
            "resolver": "cistron.data.resolver",
            "async": False,
            "mode": "offline_fast",
            "http_timeout_s": cut,
