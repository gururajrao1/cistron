"""
Clinical profile generator & ingestion for VOIDSIGNAL real-world benchmarking.
...
:class:`~voidsignal.patient_profile.PatientSignalingNetwork` instances.
"""
...
def clinical_specs_to_vcf_text(
    ...
    *,
    ...
    lines = [
        "##fileformat=VCFv4.2",
        "##source=VOIDSIGNAL_clinical_benchmark",
...
def write_multihit_vcf(
    ...
    *,
    ...
    target = Path(path) if path is not None else Path(tempfile.gettempdir()) / "voidsignal_multihit.vcf"
    target.write_text(
...
def write_expression_tsv(
    ...
    *,
    ...
    target = Path(path) if path is not None else Path(tempfile.gettempdir()) / "voidsignal_expression.tsv"
...
def parse_clinical_vcf_string(vcf_text: str, *, tmp_name: str = "inline.vcf") -> Tuple[List[VariantRecord], List[str]]:
    ...
    path = Path(tempfile.gettempdir()) / f"voidsignal_{tmp_name}"
