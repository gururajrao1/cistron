"""
Knowledge-integration clients for VOIDSIGNAL Phase 2.

Asynchronous, rate-limited wrappers around public biology APIs that materialise
Phase 1 :class:`~voidsignal.topology.SignalingNetwork` objects and enrich
:class:`~voidsignal.components.Protein` nodes with sequence / domain context.
...
from voidsignal.cache import ResponseCache
...
class AsyncHTTPClient:
    ...
    def __init__(
        ...
        *,
        ...
        per: float = 1.0,
        timeout: float = 30.0,
        user_agent: str = "VOIDSIGNAL/0.2 (+https://github.com/voidsignal)",
        max_retries: int = 4,
        backoff: float = 0.75,
    ...
    def _cache_lookup(self, url: str, namespace: Optional[str] = None) -> Optional[HTTPResponse]:
        ...
        return HTTPResponse(
            ...
            status=status,
            body=body,
            headers={"X-VOIDSIGNAL-Cache": "HIT"},
            from_cache=True,
        )
...
@dataclass
class UniProtRecord:
    """Subset of UniProtKB fields consumed by VOIDSIGNAL."""
    ...
    def to_protein(self, *, concentration: float = 0.1) -> Protein:
        """Materialise a Phase 1 :class:`~voidsignal.components.Protein`."""
