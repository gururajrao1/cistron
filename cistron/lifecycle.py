...
class CistronPipelineConfig(BaseModel):
...
class CistronPipelineResult(BaseModel):
    """Validated end-to-end artefacts from :class:`CistronPipeline`."""
...
class CistronPipeline:
    """
    Master orchestrator for the Cistron platform lifecycle.
    ...
    def __init__(
        self,
        config: Optional[Union[CistronPipelineConfig, Mapping[str, Any]]] = None,
        *,
        graph: Optional[CausalActivityGraph] = None,
    ) -> None:
        if config is None:
            self.config = CistronPipelineConfig()
        elif isinstance(config, CistronPipelineConfig):
            self.config = config
        else:
            self.config = CistronPipelineConfig.model_validate(dict(config))
    ...
    def run(self) -> CistronPipelineResult:
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="cistron.pipeline",
        description=(
            "Cistron end-to-end runner: ingest → perturb → Hill-cube ODE → "
...
def render_export(result: CistronPipelineResult, fmt: str) -> str:
    f = fmt.lower()
    if f in ("markdown", "md"):
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    """CLI entry: ``python -m cistron.pipeline`` / ``cistron-run``."""
